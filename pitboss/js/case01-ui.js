...
(function (global) {
  ...
  class Case01UI {
    ...
    _renderNotebook(snap) {
      ...
      let html = '';
      if (h <= 1) {
        html += `<span>RC <strong>${snap.runningCount}</strong></span>`;
        html += `<span>Decks <strong>${snap.decksRemaining.toFixed(2)}</strong></span>`;
        html += `<span>TC <strong>${snap.trueCount}</strong></span>`;
      } else if (h === 2) {
        html += `<span>RC <strong>${snap.runningCount}</strong></span>`;
        html += `<span>Decks <strong>${snap.decksRemaining.toFixed(2)}</strong></span>`;
        html += `<span>TC <em>(you)</em></span>`;
      } else if (h === 3) {
        html += `<span>Decks <strong>${snap.decksRemaining.toFixed(2)}</strong></span>`;
      } else {
        html += `<span class="muted">Notebook blank</span>`;
      }
      this.el.notebook.innerHTML = html + `<div class="note" id="notebookNote">${snap.notebookNote || ''}</div>`;
    }
