...
/** Safe localStorage helpers — iframe about:blank / privacy mode can throw SecurityError. */
function cqSoundStorageGet() {
  try {
    return localStorage.getItem('cq-sound');
  } catch (_) {
    return null;
  }
}
function cqSoundStorageSet(value) {
  try {
    localStorage.setItem('cq-sound', value);
  } catch (_) { /* storage unavailable */ }
}

function createSoundsNoopStub() {
  return {
    enabled: true,
    setEnabled() {},
    init() {},
...
class SoundFX {
  constructor() {
    this.ctx = null;
    // Default enabled=true when storage is missing/blocked
    this.enabled = cqSoundStorageGet() !== 'false';
  }
  init() {
  ...
  setEnabled(on) {
    this.enabled = !!on;
    cqSoundStorageSet(on ? 'true' : 'false');
  }
...
try {
  ...
  console.warn('CountQuest: SoundFX init failed — using silent stub', err);
  Sounds = createSoundsNoopStub();
}
