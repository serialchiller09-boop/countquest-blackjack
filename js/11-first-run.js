...
(function () {
  ...
  function isReturningPlayer() {
    const save = window.app && window.app.save;
    if (!save) return false;
    const st = save.stats || {};
    return (st.handsPlayed || 0) > 0 || !!(save.tutorial && save.tutorial.completed) || !!save.sessionActive;
  }

  /** Close welcome only — do NOT open daily reward (that stole Start tutorial / Sit a table). */
  ...
  function maybeShowDailyOnMenu() {
    try {
      ...
      const phase = window.app.phase || 'menu';
  ...
  function ensureMarkup() {
    ...
    const btnTut = document.getElementById('cq-first-run-tutorial');
    const btnTable = document.getElementById('cq-first-run-table');
    const btnSkip = document.getElementById('cq-first-run-skip');
    if (btnTut) {
      btnTut.addEventListener('click', startTutorial);
      btnTut.setAttribute('onclick', 'window.__cqFirstRun&&window.__cqFirstRun.startTutorial();return false;');
    }
    if (btnTable) {
      btnTable.addEventListener('click', sitTable);
      btnTable.setAttribute('onclick', 'window.__cqFirstRun&&window.__cqFirstRun.sitTable();return false;');
    }
    if (btnSkip) {
      btnSkip.addEventListener('click', skipToMenu);
      btnSkip.setAttribute('onclick', 'window.__cqFirstRun&&window.__cqFirstRun.skipToMenu();return false;');
    }
    dlg.addEventListener('cancel', (e) => { e.preventDefault(); skipToMenu(); });
  }
  ...
  function maybeShow() {
    ...
    closeOtherDialogs();
    setTimeout(closeOtherDialogs, 450);
    setTimeout(closeOtherDialogs, 900);
  }
